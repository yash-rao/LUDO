...
class Dice:
    ...
    def set(cls, flag):
        ...
        if colors[cls.chance][0].is_player_won():
            ...
            next_label = tk.Label(ludo.get_frame(), text='{} turn'.format(turn[flag]), font=('Consolas', 20), width=30, height=3,
                            borderwidth=3, relief=tk.SUNKEN,foreground='green',background='black')
...
def startgame():
    ...
    start_label = tk.Label(ludo.get_frame(), text='! So, Let\'s Begin with {}'.format(turn[0]), font=('Consolas', 20),width=40, height=3, borderwidth=3, relief=tk.SUNKEN,foreground='green',background='black')
    start_label.place(x=20, y=100)
    top.destroy()

def create_enterpage():
    enter_label = tk.Label(top, text='Enter Your Nickname!', font=('Jokerman', 20), width=30, height=3,borderwidth=3, relief=tk.RAISED,background='#151B54',foreground='orange')
    enter_label.place(x=20, y=20)

    enter_button = tk.Button(top, text='Play Game', command=startgame, width=15, height=2, font=('Verdana',16),background='black',foreground='white')
    enter_button.place(x=210, y=500)

    # players_name = {
    # }

    for i in range(2):
        temp = tk.Entry(top, width=15,bd='4')
        temp.place(x=87, y=220 + i*180 + 20)
        players.append(temp)


    for i in range(2):
        temp = tk.Entry(top, width=15,bd='4')
        temp.place(x=387, y=400 - i*180 + 20)
        players.append(temp)
...
players = []
root = tk.Tk()
width = root.winfo_screenwidth()    # SETTING MAX WIDTH AND HEIGHT OF MAIN WINDOW
height = root.winfo_screenheight()
root.geometry('{}x{}'.format(width, height)) #CREATED OUR MAIN WINDOW
root.title('Ludo By Yash and Mehul') # GIVING A TITLE
